...
private:
  virtual void request_stratege(std::string &url, std::string &content,
                                MYSQL *mysql,
...
public:
  way_manager() = default;
  ~way_manager() = default;
  void do_way(std::string &url, std::string &content, MYSQL *mysql,
              std::string &response_content) {
    auto &creators = get_creator();
    auto it = creators.find(url);
    if (it != creators.end() && it->second != nullptr) {
      it->second->request_stratege(url, content, mysql, response_content);
    }
  }
...
public:
  auto_register() {
    way_manager::register_creator(std::unique_ptr<T>(new T()));
  }
};
...
public:
  login_way() = default;
  void request_stratege(std::string &url, std::string &content, MYSQL *mysql,
